...
class SpeakerEncoder(nn.Module):
    def __init__(self, n_channels, n_resnetblocks, n_speakers):
        super().__init__()
        self.n_channels = n_channels
        ...
        self.resnetblocks = nn.ModuleList(
            [ResNetBlock(n_channels) for _ in range(n_resnetblocks)]
        )
        self.conv2 = nn.Conv1d(n_channels, n_channels, kernel_size=1)
        self.linear = nn.Linear(n_channels, n_speakers)
...
class SpeakerExtractor(nn.Module):
    def __init__(self, n_channels, n_tcnblocks):
        super().__init__()
        self.n_channels = n_channels
        ...
        self.ln1 = nn.LayerNorm(n_channels * 3)
        self.conv1 = nn.Conv1d(n_channels * 3, n_channels, kernel_size=1)
        self.stacked_tcnblocks1 = nn.ModuleList(
            [TCNBlockRef(n_channels, dilation=2**0, ref_dim=n_channels),
            *[TCNBlock(n_channels, dilation=2**i) for i in range(1, n_tcnblocks)]]
        )
        self.stacked_tcnblocks2 = nn.ModuleList(
            [TCNBlockRef(n_channels, dilation=2**0, ref_dim=n_channels),
            *[TCNBlock(n_channels, dilation=2**i) for i in range(1, n_tcnblocks)]]
        )
        self.stacked_tcnblocks3 = nn.ModuleList(
            [TCNBlockRef(n_channels, dilation=2**0, ref_dim=n_channels),
            *[TCNBlock(n_channels, dilation=2**i) for i in range(1, n_tcnblocks)]]
        )
        self.stacked_tcnblocks4 = nn.ModuleList(
            [TCNBlockRef(n_channels, dilation=2**0, ref_dim=n_channels),
            *[TCNBlock(n_channels, dilation=2**i) for i in range(1, n_tcnblocks)]]
        )
    ...
    def forward(self, x, ref):
        y = self.ln1(x.transpose(1, 2)).transpose(1, 2)
        y = self.conv1(y)
        y = self.apply_stacked_tcnblocks(y, ref, self.stacked_tcnblocks1)
        y = self.apply_stacked_tcnblocks(y, ref, self.stacked_tcnblocks2)
        y = self.apply_stacked_tcnblocks(y, ref, self.stacked_tcnblocks3)
        y = self.apply_stacked_tcnblocks(y, ref, self.stacked_tcnblocks4)
...
class SpexPlus(nn.Module):
    def __init__(self, 
        ...
        self.speaker_encoder = SpeakerEncoder(n_channels, n_resnetblocks, n_speakers)
        self.speaker_extractor = SpeakerExtractor(n_channels, n_tcnblocks)
