...
class ShortCausalInferencer:
    def __init__(self,
        ...
        assert type in ["simple", "rnn", "gru", "cache", "sp_channels", "gru_v1", "gru_v2", "gru_v3"]
        self.type = type
    ...
    def simple_forward(self, batch):
        ...
        for i in range(n_chunks):
            ...
            if i == 0:
                s1, ref_vec = self.model(chunk, batch["ref"], False, one_chunk=True)
                batch["s1"].append(s1)
            else:
                s1 = self.model(chunk, ref=None, have_relevant_speakers=False, 
    ...
    def memory_forward(self, batch):
        ...
        for i in range(n_chunks):
            chunk = batch["mix_chunks"][i: i + 1, :]
            if i == 0:
                s1, memory, ref_vec = self.model(chunk, batch["ref"], False, one_chunk=True)
                batch["s1"].append(s1)
            else:
                s1, memory = self.model(chunk, ref=None, have_relevant_speakers=False, 
                                        ref_vec=ref_vec, memory=memory, one_chunk=True)
                batch["s1"].append(s1)
        batch["s1"] = torch.cat(batch["s1"], dim=0)
        length = batch["lens"][0]
        batch["s1"] = self.streamer.apply_overlap_add_method(batch["s1"], n_chunks)
    ...
    def run(self):
        for batch_idx, batch in enumerate(
            ...
            if self.type in ["simple", "cache"]:
                batch = self.simple_forward(batch)
            elif self.type in ["rnn", "gru", "sp_channels", "gru_v1", "gru_v2", "gru_v3"]:
                batch = self.memory_forward(batch)
