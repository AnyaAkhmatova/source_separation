...
class SimpleShortCausalTrainer:
    ...
    def _log_sample(self, batch):
        ...
        mix = self._normalize_audio(batch["mix"][ind][:mix_len])
        ref = self._normalize_audio(batch["ref"][ind])
        target = self._normalize_audio(batch["target"][ind][:mix_len])
        pred = self._normalize_audio(batch["s1"][ind][:mix_len])        
